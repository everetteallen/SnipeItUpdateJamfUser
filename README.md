...
* **Jamf Pro API Role Authentication:** Uses Jamf Pro API Roles (Client ID/Secret) for secure token management.
...
5.  **Jamf Pro Authentication (via API Role):** The script authenticates with your Jamf Pro instance using a Client ID and Client Secret associated with an API Role.
6.  **Jamf Pro Update:** With the serial number, the script then updates the computer record's user and location fields in Jamf Pro.
7.  **Logging:** All steps, including successes and failures, are logged to a Google Sheet.
8.  **200 OK Response:** Regardless of the outcome of the Jamf update or any internal errors, the script always responds with an HTTP 200 OK status to Snipe-IT to ensure the webhook is considered successful on Snipe-IT's side.
...
3.  **Configure Script Properties:**
    ...
    * Click **Add script property** and add the following properties, replacing the placeholder values with your actual information:
        ...
        * `JAMF_CLIENT_ID`: The Client ID for your Jamf Pro API Role (see **Jamf Pro Setup** below).
        * `JAMF_CLIENT_SECRET`: The Client Secret for your Jamf Pro API Role (see **Jamf Pro Setup** below).
...
1.  **Create an API Role:**
    ...
    * Go to **Settings** (gear icon) > **System Settings** > **API Roles and Clients**.
    * Click **+ New** to create a new API Role.
    * Give it a descriptive name (e.g., "Snipe-IT Webhook Integration").
    * **Permissions:** Configure the privileges for this role. For this script, it needs:
        * **Computers:**
            * `Read` privilege for "Computers"
            * `Update` privilege for "Computers" (specifically for "User and Location" information)
        * **API Integrations:**
            * `Create` privilege for "API Integrations" (if you want to manage API Clients via API, though we'll create it manually here)
            * `Read` privilege for "API Integrations"
    * Click **Save**.
2.  **Create an API Client:**
    * Still in **Settings** > **System Settings** > **API Roles and Clients**, go to the **API Clients** tab.
    * Click **+ New**.
    * Give it a descriptive name (e.g., "Snipe-IT Client").
    * Select the **API Role** you just created (e.g., "Snipe-IT Webhook Integration").
    * **Authorization Type:** Choose `Client Credentials`.
    * Click **Save**.
    * **Important:** Jamf Pro will display the **Client ID** and **Client Secret** *only once*. Copy both of these values immediately.
    * These are your `JAMF_CLIENT_ID` and `JAMF_CLIENT_SECRET` for your Google Apps Script properties.
...
* **"Jamf OAuth Token Error" / "Failed to get Jamf token via OAuth":**
    * Verify `JAMF_URL`, `JAMF_CLIENT_ID`, and `JAMF_CLIENT_SECRET` are correctly set in your Apps Script properties.
    * Ensure the API Client in Jamf Pro is enabled and associated with an API Role that has sufficient permissions (at least `Read` for "API Integrations" and `Create` for "API Integrations" if you were to manage them via API, but for token generation, primarily the correct Client ID/Secret and associated Role permissions for computers).
    * Double-check the API Role's permissions in Jamf Pro to ensure it has the necessary `Read` and `Update` privileges for "Computers."
...
* **"No Jamf computer found for serial":**
    ...
    * Ensure the Jamf Pro API Role has permissions to search for computers.

* **"Jamf update failed":**
    * Check your Jamf Pro API Role's permissions. It needs permission to update computer inventory records (specifically the User & Location section).
